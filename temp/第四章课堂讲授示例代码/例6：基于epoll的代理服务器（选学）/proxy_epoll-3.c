...
程序中，int ggg;开头的那几行示例代码，就是可加入加密/认证的地方，大家可以采用任何自己喜欢的方式
进行加密/认证，替换那几行代码———无论是常见的一些加解密方法（例如用openssl），还是自己开发的算法。

备注：
1. 程序使用到了c++的stl库，因此需要用g++来编译。编译命令格式：
g++ proxy_epoll-3.c proxy_epoll.h -o 输出文件名

2. 程序使用了daemon技术，即运行后进入后台，而不是占用控制台窗口。运行日志，在proxy_epoll_log
文件中。

3. 程序仅供教学演示，未提供对https的完整支持（Tips: https协议协商时较慢，用epoll的ET模式需要处理
此问题）。
